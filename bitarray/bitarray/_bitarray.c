...
static PyTypeObject Cuckoohash_Type;
...
decodetree_todict(decodetreeobject *self)
{
    ...
    prefix = newbitarrayobject(&Cuckoohash_Type, 0, default_endian);
    if (prefix == NULL)
        goto error;
...
#endif
{
    ...
    if (PyType_Ready(&Cuckoohash_Type) < 0)
        goto error;
    Py_SET_TYPE(&Cuckoohash_Type, &PyType_Type);
    Py_INCREF((PyObject *) &Cuckoohash_Type);
    PyModule_AddObject(m, "bitarray", (PyObject *) &Cuckoohash_Type);
