...
class HashTab(object):
    ...
    def hashFunc(self, s):
        x = self.hasher(str(s))          # hash twice 
        y = self.hasher(str(s) + str(x))
    ...
    def insert(self, k, d):
        '''Insert d with key k. Automatically resizes the hashtable if needed.
        
        Return false if key/data is already present and true otherwise.'''
        if self.find(k) != None:    # if already there, return False (no duplicates)
            self.delete(k)
            # return False   
    ...
    def rehash(self, size):
        self.hasher = pyhash.fnv1_32()  # get new hash functions
    ...
    def incc(self, k):
        '''Find key k and increment the value stored there.'''

        pos1, pos2 = self.hashFunc(k)               # check both positions the key/data
        x = self.__hashArray1[pos1]                 # could be in. return data if found.
        y = self.__hashArray2[pos2]                 
        if   x != None and x.key == k:  x.data += 1         # Increment the value if found
        elif y != None and y.key == k:  y.data += 1
        else:                           self.insert(k, 1)   # Otherwise, insert 1
    ...
    def items(self):
        for i in range(self.__size // 2):
            if self.__hashArray1[i] != None:
                yield self.__hashArray1[i].key, self.__hashArray1[i].data
            elif self.__hashArray2[i] != None:
                yield self.__hashArray2[i].key, self.__hashArray2[i].data
